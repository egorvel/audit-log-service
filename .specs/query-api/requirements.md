...
- **AC1.11 — Ubiquitous.** The system shall deduplicate the `actor` id list before filter evaluation and before applying the cap in AC1.13; duplicate ids shall have no effect on which events are returned.
...
- **AC3.2 — Ubiquitous.** The system shall accept any `next_cursor` value it previously emitted when submitted unchanged as the next request's `cursor` (subject to AC3.5/AC3.9b). (The cursor's internal byte layout is a server-side concern; see `design.md` §3.2.)

...
**Refs.**
- requirements AC1.2 (reformulated to set membership), AC1.11 (dedup), AC1.12 (400 on empty value / empty entry / trailing comma), AC1.13 (422 on >10 distinct ids), AC1.14 (400 on empty resource), AC3.11 (cursor `fh` match is set-based), AC3.12 (multi-actor walk: union, no dup, no loss).
...
- **AC3.12 multi-actor walk:** seed N events across 3 actors `{a1, a2, a3}`, walk `actor=a1,a2,a3` via `next_cursor` to exhaustion, assert the returned union equals the seeded subset whose `actor.id ∈ {a1,a2,a3}` (and other filters), with no duplicates and no omissions. Assert `distinct(events.id).size() == events.size()` per page and across all pages — this pins the no-duplicates clause of AC3.12 at the HTTP boundary, even though `design.md` §6.3 makes the property structural (single `actor_id` column + upstream dedup ⇒ each row matches at most one `IN` element).
